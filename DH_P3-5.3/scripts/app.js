...
function showLegend(){ try{ document.getElementById('legend-section')?.classList.remove('hidden'); }catch(e){} }
        ...
        const compareControls = document.querySelector('.mhA-filters .compare-controls');
        ...
        const searchPopover = searchChip?.querySelector('.mhA-search-popover');
        const rosterSearchInput = document.getElementById('rosterSearchInput');
        let searchDebounceId = null;
        let removeOutsideSearchHandler = null;
        let removeEscapeSearchHandler = null;
        ...
        if (searchTrigger && rosterSearchInput && searchChip) {
            const openSearch = () => {
                ...
                removeOutsideSearchHandler = () => {
                    document.removeEventListener('pointerdown', handlePointerDown);
                    removeOutsideSearchHandler = null;
                };
                removeEscapeSearchHandler = () => {
                    document.removeEventListener('keydown', handleEscape);
                    removeEscapeSearchHandler = null;
                };
            };
            ...
            const closeSearch = () => {
                ...
                if (removeOutsideSearchHandler) {
                    removeOutsideSearchHandler();
                }
                if (removeEscapeSearchHandler) {
                    removeEscapeSearchHandler();
                }
            };
            ...
            rosterSearchInput.addEventListener('blur', () => {
                setTimeout(() => {
                    if (!searchChip.contains(document.activeElement) && !rosterSearchInput.value.trim()) {
                        closeSearch();
                    }
                }, 120);
            });
        
            searchChip.addEventListener('transitionend', () => {
                if (!searchChip.classList.contains('is-open')) {
                    searchTrigger.blur();
                }
            });
        ...
        const PLAYER_STATS_SHEETS = { season: 'SZN', seasonRanks: 'SZN_RKs', weeks: { 1: 'WK1', 2: 'WK2', 3: 'WK3' } };
    ...
    hideLegend();
        ...
        function updateCompareButtonState() {
            ...
            compareControls?.classList.toggle('is-visible', count > 0 || state.isCompareMode);
