(function () {
  ...
  function renderSunburst() {
    ...
    ring1Segments.forEach((segment) => {
      ...
      const path = createSVG('path', {
        ...
        fill: hexToRgba(color, INNER_RING_OPACITY),
        stroke: colors.bg,
    ...
    ring2Segments.forEach((segment) => {
      ...
      const path = createSVG('path', {
        ...
        fill: hexToRgba(parentColor, 0.42),
        stroke: colors.bg,
  ...
  function renderDraftPositional() {
    ...
    const labelOffsets = width < 560 ? [-18, -10, 6, 16] : [-20, -12, 6, 18];
    const labelStroke = 'rgba(8, 11, 19, 0.8)';
    ...
    DRAFT_SERIES.forEach((series, seriesIndex) => {
      ...
      points.forEach((point) => {
        ...
        g.appendChild(createSVG('text', {
          x: point.x,
          y: point.y + (labelOffsets[seriesIndex] || -10),
          fill: colors.text,
          'font-size': '10',
          'font-weight': '600',
          'text-anchor': 'middle',
          'paint-order': 'stroke',
          stroke: labelStroke,
          'stroke-width': width < 560 ? '1.8' : '1.6'
