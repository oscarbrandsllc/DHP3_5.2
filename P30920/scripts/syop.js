(function () {
  ...
  function renderBarChart() {
    ...
    if (!syopChartState.activePosition) {
      syopChartState.activePosition = POSITION_CONFIG[0]?.key || null;
    }
    ...
    POSITION_CONFIG.forEach((config) => {
      ...
      const chip = createEl('button', {
        ...
        'aria-label': `${config.key} — ${config.label}`
      },
  ...
  function drawViolinPlot(plotContainer, config, tooltip, rootContainer) {
    ...
    const height = isCompact ? 212 : 248;
    const margin = isCompact
      ? { top: 18, right: 16, bottom: 44, left: 24 }
      : { top: 22, right: 20, bottom: 50, left: 28 };

    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const axisPadding = isCompact ? 44 : 48;
    const areaHeight = chartHeight - axisPadding;
    const axisY = areaHeight;
    ...
    const densityScale = (value) => (value / maxDensity) * Math.max(12, areaHeight - 28);
    ...
    if (density.length > 1) {
      const ridgePoints = density.map((point) => ({
        x: scaleX(point.x),
        y: axisY - densityScale(point.y)
      }));
      const pathCommands = [];
      pathCommands.push(`M ${ridgePoints[0].x} ${axisY}`);
      ridgePoints.forEach((point) => {
        pathCommands.push(`L ${point.x} ${point.y}`);
      });
      pathCommands.push(`L ${ridgePoints[ridgePoints.length - 1].x} ${axisY}`);
      pathCommands.push('Z');
      ...
      g.appendChild(createSVG('path', {
        ...
        style: {
          fill: hexToRgba(config.color, 0.28),
          stroke: hexToRgba(config.color, 0.62)
        }
      }));
    }
    ...
    const nodes = createBeeswarmNodes(values, 0, areaHeight);
    const dotRadius = isCompact ? 4 : 5.5;

    nodes.forEach((node) => {
      const player = sortedPlayers[node.index];
      const cx = scaleX(node.value);
      const cy = Math.max(dotRadius, Math.min(areaHeight - dotRadius, centerY + node.offset));
