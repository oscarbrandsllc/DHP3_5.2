(function () {
  ...
  function createBeeswarmNodes(values, minY, maxY) {
    ...
    values.forEach((value, index) => {
      ...
      const offsetLevel = bin.length;
      bin.push(offsetLevel);
      bins.set(key, bin);
      nodes.push({ value, index, offsetLevel });
    });

    const maxLevel = nodes.reduce((acc, node) => Math.max(acc, node.offsetLevel), 0);
    const available = Math.max(0, maxY - minY);
    const stepSize = available / Math.max(1, maxLevel + 1);
    const minOffset = minY - maxY;

    return nodes.map((node) => ({
      value: node.value,
      index: node.index,
      offset: Math.max(minOffset, -node.offsetLevel * stepSize)
    }));
  }
  ...
  function drawViolinPlot(plotContainer, config, tooltip, rootContainer) {
    ...
    const margin = isCompact
      ? { top: 18, right: 12, bottom: 40, left: 26 }
      : { top: 22, right: 18, bottom: 46, left: 32 };
    ...
    const densityScale = (value) => (value / maxDensity) * Math.max(18, chartHeight - 28);
    ...
    if (density.length > 1) {
      ...
      const ridge = density.map((point) => ({
        x: scaleX(point.x),
        y: Math.max(0, baselineY - densityScale(point.y))
      }));
      const pathCommands = [];
      pathCommands.push(`M ${ridge[0].x} ${baselineY}`);
      ridge.forEach((point) => {
        pathCommands.push(`L ${point.x} ${point.y}`);
      });
      pathCommands.push(`L ${ridge[ridge.length - 1].x} ${baselineY}`);
      pathCommands.push('Z');
    ...
    const nodes = createBeeswarmNodes(values, dotRadius, chartHeight - dotRadius);
    const baseY = chartHeight - dotRadius;

    nodes.forEach((node) => {
      const player = sortedPlayers[node.index];
      const cx = scaleX(node.value);
      const cy = Math.max(dotRadius, Math.min(baseY, baseY + node.offset));
