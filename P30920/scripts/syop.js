(function () {
  ...
  const SERIES_ORDER = DRAFT_SERIES.map((series) => series.key);
  ...
  function renderDraftPositional() {
    ...
    const labelEntries = [];
    const entriesByRound = rounds.map(() => []);
    ...
    DRAFT_SERIES.forEach((series) => {
      ...
      points.forEach((point) => {
        ...
        entriesByRound[point.roundIndex].push(entry);
      });
    });

    const aboveOffsets = isCompact ? [-26, -14] : [-30, -18];
    const belowOffsets = isCompact ? [16, 28] : [20, 34];
    const middleOffset = isCompact ? 4 : 6;
    const minGapForMiddle = 9;

    entriesByRound.forEach((entries) => {
      ...
      entries.sort((a, b) => {
        ...
        const aIndex = SERIES_ORDER.indexOf(a.series.key);
        const bIndex = SERIES_ORDER.indexOf(b.series.key);
        return aIndex - bIndex;
      });

      entries.slice(0, 2).forEach((entry, index) => {
        entry.offsetY = aboveOffsets[Math.min(index, aboveOffsets.length - 1)];
      });

      entries.slice(2).forEach((entry, index) => {
        entry.offsetY = belowOffsets[Math.min(index, belowOffsets.length - 1)];
      });
      ...
      if (entries.length >= 3) {
        ...
        if (top && third) {
          const gap = Math.abs(top.value - third.value);
          if (gap >= minGapForMiddle) {
            third.offsetY = middleOffset;
          }
        }
      }
    });
