(function () {
  ...
  function placeKeyLegend() {
    const keyLegend = document.querySelector('.syop-key-legend');
    const anchor = document.getElementById('syop-key-anchor');
    const gaugesPanel = document.getElementById('syop-gauges');
    const panelsGrid = gaugesPanel?.parentElement || null;
    if (!keyLegend || !anchor || !gaugesPanel || !panelsGrid) return;

    const isDesktop = keyLegendQuery ? keyLegendQuery.matches : window.matchMedia('(min-width: 1024px)').matches;

    if (isDesktop) {
      if (keyLegend.parentElement !== panelsGrid || keyLegend.previousElementSibling !== gaugesPanel) {
        const nextSibling = gaugesPanel.nextElementSibling;
        if (nextSibling) {
          panelsGrid.insertBefore(keyLegend, nextSibling);
        } else {
          panelsGrid.appendChild(keyLegend);
        }
      }
    } else if (keyLegend.parentElement !== anchor.parentElement || keyLegend.previousElementSibling !== anchor) {
      anchor.insertAdjacentElement('afterend', keyLegend);
    }

    keyLegend.classList.toggle('syop-key-inline', isDesktop);
  }
  ...
  function init() {
    ...
    keyLegendQuery = window.matchMedia('(min-width: 1024px)');
    if (typeof keyLegendQuery.addEventListener === 'function') {
      keyLegendQuery.addEventListener('change', placeKeyLegend);
    } else if (typeof keyLegendQuery.addListener === 'function') {
      keyLegendQuery.addListener(placeKeyLegend);
    }
    setupTabs();
    renderSunburst();
    renderBarChart();
