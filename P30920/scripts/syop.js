(function () {
  ...
  function drawSyopBarChart(plotContainer, config, distribution, tooltip, rootContainer) {
    ...
    const margin = isCompact
      ? { top: 20, right: 14, bottom: 50, left: 38 }
      : { top: 26, right: 18, bottom: 60, left: 42 };
    ...
    const yAxisAnchorX = margin.left - (isCompact ? 26 : 30);
    const yAxisAnchorY = margin.top + chartHeight / 2;
    svg.appendChild(createSVG('text', {
      x: yAxisAnchorX,
      y: yAxisAnchorY,
      class: 'syop-bar-axis-title syop-bar-axis-title-y',
      transform: `rotate(-90 ${yAxisAnchorX} ${yAxisAnchorY})`
