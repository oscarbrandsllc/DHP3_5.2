(function () {
  ...
  function renderDraftPositional() {
    ...
    roundLabelGroups.forEach((entries) => {
      ...
      const topBase = isCompact ? -34 : -36;
      const topSpacing = isCompact ? 7 : 8;
      const bottomBase = isCompact ? 28 : 30;
      const bottomSpacing = isCompact ? 7 : 8;
      const midOffset = isCompact ? 9 : 11;
      ...
      topGroup.forEach((entry, index) => {
        entry.offsetY = topBase + index * topSpacing;
      });

      bottomGroup.forEach((entry, index) => {
        entry.offsetY = bottomBase + index * bottomSpacing;
      });

      if (bottomGroup.length >= 1 && topGroup.length >= 2) {
        const transitionGap = topGroup[topGroup.length - 1].value - bottomGroup[0].value;
        if (transitionGap >= 9) {
          bottomGroup[0].offsetY = midOffset;
          bottomGroup.slice(1).forEach((entry, index) => {
            entry.offsetY = bottomBase + (index + 1) * bottomSpacing;
          });
        }
      }
    });
