(function () {
  ...
  const syopChartState = {
    activePositions: new Set(POSITION_CONFIG.map((pos) => pos.key)),
    showTable: false
  };
  ...
  function createBeeswarmNodes(values, minY, maxY) {
    const nodes = [];
    const bins = new Map();
    const step = 0.2;
    ...
    values.forEach((value, index) => {
      ...
      const level = Math.floor((bin.length + 2) / 2);
      const direction = bin.length % 2 === 0 ? 1 : -1;
      const offsetLevel = level * direction;
      bin.push(offsetLevel);
      bins.set(key, bin);
      nodes.push({ value, index, offsetLevel });
    });

    const maxLevel = nodes.reduce((acc, node) => Math.max(acc, Math.abs(node.offsetLevel)), 0) || 1;
    const scale = (maxY - minY) / (2 * maxLevel || 1);

    return nodes.map((node) => ({
      value: node.value,
      index: node.index,
      offset: node.offsetLevel * scale
    }));
  }
  ...
  function renderBarChart() {
    ...
    POSITION_CONFIG.forEach((config) => {
      const isActive = syopChartState.activePositions.has(config.key);
      ...
      createEl('span', { class: 'chip-sublabel' }, config.label));

      chip.addEventListener('click', () => {
        if (isActive && syopChartState.activePositions.size === 1) {
          return;
        }
        if (syopChartState.activePositions.has(config.key)) {
          syopChartState.activePositions.delete(config.key);
        } else {
          syopChartState.activePositions.add(config.key);
        }
        renderBarChart();
      });
    ...
    const activeConfigs = POSITION_CONFIG.filter((config) => syopChartState.activePositions.has(config.key));
    ...
    activeConfigs.forEach((config) => {
      ...
      const header = createEl('header', { class: 'syop-violin-header' },
        createEl('div', { class: 'syop-violin-title-block' },
          createEl('h4', { class: 'syop-violin-title' }, config.label),
        ...
        createEl('div', { class: 'syop-violin-chip', style: { '--chip-accent': config.color } },
          createEl('span', { class: 'chip-label' }, '≥2 SYOP'),
          createEl('span', { class: 'chip-value' }, `${Math.round((metrics?.shareTwoPlus || 0) * 100)}%`)
        )
      );
      ...
      drawViolinPlot(plot, config, tooltip, container);
    });
  }
  ...
  function drawViolinPlot(plotContainer, config, tooltip, rootContainer) {
    ...
    const margin = isCompact
      ? { top: 22, right: 18, bottom: 42, left: 34 }
      : { top: 26, right: 28, bottom: 48, left: 44 };
    ...
    const densityScale = (value) => (value / maxDensity) * (chartHeight / 2 - 18);
    ...
    if (density.length > 1) {
      const upper = density.map((point) => ({
        x: scaleX(point.x),
        y: centerY - densityScale(point.y)
      }));
      const lower = density.slice().reverse().map((point) => ({
        x: scaleX(point.x),
        y: centerY + densityScale(point.y)
      }));
      const pathCommands = [];
      pathCommands.push(`M ${upper[0].x} ${upper[0].y}`);
    ...
    const nodes = createBeeswarmNodes(values, 0, chartHeight);
    const dotRadius = isCompact ? 4 : 5.5;

    nodes.forEach((node) => {
      const player = sortedPlayers[node.index];
      const cx = scaleX(node.value);
      const cy = Math.max(dotRadius, Math.min(chartHeight - dotRadius, centerY + node.offset));
