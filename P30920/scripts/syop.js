(function () {
  ...
  const SERIES_ORDER = ['QB', 'RB', 'TE', 'WR'];
  ...
  function renderDraftPositional() {
    ...
    const desktopOffsets = [-24, -14, 8, 22];
    const compactOffsets = [-18, -10, 6, 18];

    entriesByRound.forEach((entries) => {
      if (entries.length === 0) return;

      const sorted = entries.slice().sort((a, b) => {
        if (b.point.value !== a.point.value) {
          return b.point.value - a.point.value;
        }
        const orderA = SERIES_ORDER.indexOf(a.series.key);
        const orderB = SERIES_ORDER.indexOf(b.series.key);
        return orderA - orderB;
      });

      const offsets = isCompact ? compactOffsets : desktopOffsets;
      sorted.forEach((entry, index) => {
        entry.offsetY = offsets[index] ?? 0;
      });

      if (sorted.length >= 4) {
        const midGap = Math.abs(sorted[1].point.value - sorted[2].point.value);
        if (midGap >= 9) {
          sorted[2].offsetY = isCompact ? 0 : 2;
        }
      }
    });
