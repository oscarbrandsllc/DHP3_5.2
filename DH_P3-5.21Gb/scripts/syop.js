(function () {
  ...
  function formatStat(stat) {
    const cleaned = typeof stat === 'string' ? stat.replace(/[^0-9.\-]/g, '') : stat;
    const num = Number.parseFloat(cleaned);
    if (!Number.isFinite(num)) return String(stat);
  ...
  function renderSunburst() {
    ...
    const fontSize = (value, floor = 14) => Math.max(value * scale * 1.32, floor);
    ...
    ring2Segments.forEach((segment) => {
      ...
      if (stat) {
        const formatted = formatStat(stat);
  ...
  function renderGauges() {
    ...
    GAUGES.forEach((gauge) => {
      ...
      gaugeWrapper.appendChild(createEl('span', { class: 'syop-gauge-footnote' }, 'Avg SYOP · yrs'));
