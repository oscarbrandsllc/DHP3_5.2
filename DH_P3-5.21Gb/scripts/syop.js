(function () {
  ...
  const SUNBURST = {
    labels: [
      ...
      'TE',
      'Strong Peak Λ (7.2)',
      'Baseline Output Λ (2.3)',
      'Strong Peak Mode (6.0)',
      'Baseline Output Mode (1.0)',
      'Strong Peak Λ (3.41)',
      'Baseline Output Λ (2.2)',
      'Strong Peak Mode (0.7)',
      'Baseline Output Mode (1.7)',
      'Strong Peak Λ (4.9)',
      'Baseline Output Λ (2.9)',
      'Strong Peak Mode (3.0)',
      'Baseline Output Mode (2.0)',
      'Strong Peak Λ (4.0)',
      'Baseline Output Λ (3.5)',
      'Strong Peak Mode (2.0)',
      'Baseline Output Mode (3.0)'
    ],
    parents: [
      '',
      'SYOP Distribution',
  ...
  function renderSunburst() {
    ...
    ring2Segments.forEach((segment, idx) => {
      ...
      const tspanMain = createSVG('tspan', { 'font-size': '16', 'font-weight': '800' }, document.createTextNode(main));
    ...
    const subtitle = createSVG('text', {
      ...
      'text-anchor': 'middle',
      'dominant-baseline': 'middle'
    }, document.createTextNode('Positional Balance'));
  ...
  function renderGauges() {
    ...
    GAUGES.forEach((gauge) => {
      ...
      const label = createEl('div', { class: 'syop-gauge-label' },
        createEl('span', { class: 'gauge-title' }, `${gauge.key} Avg SYOP`),
        createEl('span', { class: 'gauge-meta' }, 'Years of production')
      );
  ...
  function setupTabs() {
    const buttons = Array.from(document.querySelectorAll('.syop-tab-button'));
    const panels = Array.from(document.querySelectorAll('.syop-tab-panel'));
    ...
    buttons.forEach((button) => {
      ...
      button.addEventListener('keydown', (event) => {
        ...
        nextButton.focus();
        activate(nextButton.dataset.tabTarget);
      });
    });

    const initialTarget = buttons.find((button) => button.classList.contains('active'))?.dataset.tabTarget
  ...
  function renderDraftOverall() {
    ...
    DRAFT_OVERALL.forEach((row, index) => {
      ...
      const barWidth = Math.min(54, groupWidth * 0.6);
  ...
  function renderDraftPositional() {
    ...
    yTicks.forEach((tick) => {
      ...
      g.appendChild(createSVG('line', {
        ...
        y1: y,
        y2: y,
        stroke: tick === 0 ? 'rgba(255,255,255,0.12)' : 'rgba(255,255,255,0.06)'
      }));
      g.appendChild(createSVG('text', {
        x: -12,
        y: y + 4,
