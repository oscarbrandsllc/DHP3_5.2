(function () {
  ...
  const colors = {
    ...
    wr: '#60A5FA',
    te: '#C084FC',
    draftGradientTop: '#22D3EE',
    draftGradientMid: '#A855F7',
    draftGradientBottom: '#EC4899',
    draftQB: '#F472B6',
    draftRB: '#22C55E',
    draftTE: '#A855F7',
    draftWR: '#22D3EE'
  ...
  const DRAFT_SERIES = [
    { key: 'QB', color: colors.draftQB },
    { key: 'RB', color: colors.draftRB },
    { key: 'TE', color: colors.draftTE },
    { key: 'WR', color: colors.draftWR }
  ];
  ...
  function renderSunburst() {
    ...
    const size = Math.min(baseSize, Math.max(320, containerWidth));
    const scale = size / baseSize;
    const pad = 36 * scale;
    const cx = size / 2;
    const cy = size / 2;
    const inner1 = 112 * scale;
    const outer1 = 190 * scale;
    const inner2 = 206 * scale;
    const outer2 = 284 * scale;
    const centerRadius = 86 * scale;
    const textStroke = 'rgba(11, 14, 22, 0.55)';
    const fontSize = (value, floor = 9) => Math.max(value * scale, floor);
    ...
    ring1Segments.forEach((segment) => {
      ...
      const text = createSVG('text', {
        ...
        'font-size': fontSize(22, 14),
        'font-weight': '800',
        'paint-order': 'stroke',
        stroke: textStroke,
        'stroke-width': (0.52 * scale).toFixed(3)
      });
    ...
    ring2Segments.forEach((segment) => {
      ...
      const label = createSVG('text', {
        ...
        'text-anchor': 'middle',
        'dominant-baseline': 'middle',
        'font-size': fontSize(18, 12),
        'font-weight': '700',
        'paint-order': 'stroke',
        stroke: textStroke,
        'stroke-width': (0.48 * scale).toFixed(3)
      });
      label.appendChild(document.createTextNode(segment.node.label));
      const detail = segment.node.subtitle || segment.node.stat;
      if (detail) {
    ...
    const titleTop = createSVG('text', {
      ...
      y: cy - 20 * scale,
      fill: colors.text,
      'font-size': fontSize(19, 13),
      'font-weight': '800',
      'text-anchor': 'middle',
      'paint-order': 'stroke',
      stroke: textStroke,
      'stroke-width': (0.5 * scale).toFixed(3)
  ...
  function renderBarChart() {
    ...
    const width = containerWidth > 0 ? Math.max(containerWidth, 320) : 360;
    const height = width < 520 ? 296 : 336;
    const margin = width < 520
      ? { top: 30, right: 14, bottom: 50, left: 40 }
      : { top: 36, right: 22, bottom: 58, left: 56 };
    ...
    SYOP_DATA.forEach((row, index) => {
      ...
      if (barState.stacked) {
        const barWidth = Math.min(50, groupWidth * 0.6);
        let cumulative = 0;
        ...
        const totalLabel = createSVG('text', {
          x: labelX,
          y: chartHeight - (cumulative / niceMax) * chartHeight - 8,
          fill: colors.text,
          'font-size': '11',
          'text-anchor': 'middle',
          'font-weight': '700'
        ...
        const barWidth = Math.min(34, (groupWidth - innerGap) / activeSeries.length);
  ...
  function renderDraftOverall() {
    ...
    const containerWidth = container.clientWidth || 0;
    const width = containerWidth > 0 ? Math.max(containerWidth, 320) : 360;
    const height = width < 540 ? 276 : 316;
    const margin = width < 540
      ? { top: 26, right: 14, bottom: 50, left: 44 }
      : { top: 32, right: 20, bottom: 56, left: 58 };
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const svg = createSVG('svg', {
      viewBox: `0 0 ${width} ${height}`,
    ...
    gradient.appendChild(createSVG('stop', { offset: '0%', 'stop-color': colors.draftGradientTop }));
    gradient.appendChild(createSVG('stop', { offset: '55%', 'stop-color': colors.draftGradientMid }));
    gradient.appendChild(createSVG('stop', { offset: '100%', 'stop-color': colors.draftGradientBottom }));
    defs.appendChild(gradient);
    svg.appendChild(defs);
    ...
    DRAFT_OVERALL.forEach((row, index) => {
      const baseX = index * groupWidth;
      const barWidth = Math.min(60, groupWidth * 0.68);
  ...
  function renderDraftPositional() {
    ...
    const margin = width < 560
      ? { top: 54, right: 22, bottom: 50, left: 60 }
      : { top: 50, right: 28, bottom: 58, left: 68 };
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const svg = createSVG('svg', {
      viewBox: `0 0 ${width} ${height}`,
