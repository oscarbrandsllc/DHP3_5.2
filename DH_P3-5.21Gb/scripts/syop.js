(function () {
  ...
  const SUNBURST_NODES = [
    ...
    { id: 'qb', parent: 'root', label: 'QB', value: 16.46, series: 'QB' },
    ...
    { id: 'rb', parent: 'root', label: 'RB', value: 9.8, series: 'RB' },
    ...
    { id: 'wr', parent: 'root', label: 'WR', value: 12.82, series: 'WR' },
    ...
    { id: 'te', parent: 'root', label: 'TE', value: 12.5, series: 'TE' },
  ...
  function renderSunburst() {
    ...
    const constrained = Math.max(320, containerWidth - 8);
    const size = Math.min(baseSize, constrained);
    const rawScale = size / baseSize;
    const scale = Math.pow(rawScale, 0.92);
    const pad = 26 * scale;
    const cx = size / 2;
    const cy = size / 2;
    const inner1 = 108 * scale;
    const outer1 = 174 * scale;
    const inner2 = 190 * scale;
    const outer2 = 256 * scale;
    const centerRadius = 82 * scale;
    const textStroke = 'rgba(9, 12, 22, 0.62)';
    const fontSize = (value, floor = 14) => Math.max(value * scale * 1.25, floor);
    ...
    const titleTop = createSVG('text', {
      ...
      y: cy - 32 * scale,
      fill: colors.text,
      'font-size': fontSize(26, 18),
      'font-weight': '800',
      'text-anchor': 'middle',
      'paint-order': 'stroke',
      stroke: textStroke,
      'stroke-width': Math.max(0.5, 0.68 * scale).toFixed(3)
  ...
  function renderSyopMosaic() {
    const container = document.getElementById('syop-bar-chart');
    if (!container) return;
    container.innerHTML = '';

    const scroller = createEl('div', { class: 'syop-mosaic-scroller', role: 'list' });
    const maxValue = Math.max(
      ...SYOP_DATA.flatMap((row) => SYOP_POSITIONS.map((pos) => row[pos.key] || 0))
    );

    SYOP_DATA.forEach((bucket) => {
      const card = createEl('div', {
        class: 'syop-mosaic-card',
        role: 'group',
        'aria-label': `SYOP bucket ${bucket.SYOP}`
      });

      const header = createEl('div', { class: 'syop-mosaic-header' },
        createEl('span', { class: 'mosaic-bucket' }, bucket.SYOP),
        createEl('span', { class: 'mosaic-bucket-caption' }, bucket.SYOP === '1' ? 'year' : 'years')
      );
      card.appendChild(header);

      const cells = createEl('div', { class: 'syop-mosaic-cells' });
      SYOP_POSITIONS.forEach((pos) => {
        const value = Number(bucket[pos.key] || 0);
        const fill = seriesColor(pos.label);
        const widthPercent = Math.max(10, Math.min(100, (value / maxValue) * 100));

        const cell = createEl('div', { class: 'syop-mosaic-cell' },
          createEl('div', { class: 'syop-mosaic-cell-head' },
            createEl('span', { class: 'mosaic-pos', style: { color: fill } }, pos.label),
            createEl('span', { class: 'mosaic-value' }, `${value.toFixed(1)}%`)
          ),
          createEl('div', { class: 'syop-mosaic-meter' },
            createEl('div', {
              class: 'syop-mosaic-meter-fill',
              style: {
                width: `${widthPercent}%`,
                background: `linear-gradient(90deg, ${hexToRgba(fill, 0.45)} 0%, ${fill} 85%)`
              }
            })
          )
        );
  ...
  function renderGauges() {
    ...
    GAUGES.forEach((gauge) => {
      ...
      const label = createEl('div', { class: 'syop-gauge-label' },
        createEl('span', { class: 'gauge-title', style: { color: colors.subtext } }, 'Avg SYOP span')
      );
  ...
  function renderGaugeSVG(gauge) {
    ...
    const valueText = createSVG('text', {
      ...
      'text-anchor': 'middle',
      'paint-order': 'stroke',
      stroke: 'rgba(11, 14, 22, 0.5)',
      'stroke-width': '0.6'
    }, document.createTextNode(gauge.value.toFixed(2)));
    svg.appendChild(valueText);

    const positionText = createSVG('text', {
      x: cx,
      y: cy - 34,
      fill: gauge.color,
      'font-size': '20',
      'font-weight': '700',
      'text-anchor': 'middle'
    }, document.createTextNode(gauge.key));
