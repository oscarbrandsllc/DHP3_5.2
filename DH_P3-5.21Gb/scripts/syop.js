(function () {
  ...
  function renderSunburst() {
    ...
    const textStroke = 'rgba(8, 11, 22, 0.82)';
    const fontSize = (value, floor = 11) => Math.max(value * scale, floor);
    ...
    ring1Segments.forEach((segment) => {
      ...
      const text = createSVG('text', {
        ...
        'paint-order': 'stroke',
        stroke: textStroke,
        'stroke-width': (0.7 * scale).toFixed(3)
      });
    ...
    ring2Segments.forEach((segment) => {
      ...
      const label = createSVG('text', {
        ...
        'text-anchor': 'middle',
        'dominant-baseline': 'middle',
        'font-size': fontSize(17, 13),
        'font-weight': '700',
        'paint-order': 'stroke',
        stroke: textStroke,
        'stroke-width': (0.65 * scale).toFixed(3)
      });
    ...
    const titleTop = createSVG('text', {
      ...
      'paint-order': 'stroke',
      stroke: textStroke,
      'stroke-width': (0.6 * scale).toFixed(3)
  ...
  function renderBarChart() {
    ...
    const margin = width < 520
      ? { top: 28, right: 14, bottom: 48, left: 48 }
      : { top: 34, right: 22, bottom: 58, left: 60 };
    ...
    SYOP_DATA.forEach((row, index) => {
      ...
      if (barState.stacked) {
        const barWidth = Math.min(52, groupWidth * 0.68);
        let cumulative = 0;
        ...
        const innerGap = Math.min(8, groupWidth * 0.15);
        const availableWidth = groupWidth - innerGap * (activeSeries.length - 1);
        const barWidth = Math.max(18, Math.min(36, availableWidth / activeSeries.length));
  ...
  function renderDraftOverall() {
    ...
    const margin = width < 540
      ? { top: 26, right: 14, bottom: 48, left: 48 }
      : { top: 32, right: 22, bottom: 56, left: 62 };
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const svg = createSVG('svg', {
      viewBox: `0 0 ${width} ${height}`,
    ...
    gradient.appendChild(createSVG('stop', { offset: '55%', 'stop-color': colors.accentB }));
    ...
    DRAFT_OVERALL.forEach((row, index) => {
      const baseX = index * groupWidth;
      const barWidth = Math.min(56, groupWidth * 0.6);
  ...
  function renderDraftPositional() {
    ...
    const margin = width < 560
      ? { top: 52, right: 22, bottom: 48, left: 58 }
      : { top: 50, right: 28, bottom: 56, left: 68 };
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const svg = createSVG('svg', {
      viewBox: `0 0 ${width} ${height}`,
