(function () {
  ...
  function renderSunburst() {
    ...
    const available = Math.max(320, containerWidth);
    const size = Math.max(320, Math.min(baseSize, available - 12));
    const rawScale = size / baseSize;
    const scale = Math.pow(rawScale, 0.88);
    const pad = Math.max(48, 52 * scale);
    const cx = size / 2;
    const cy = size / 2;
    const inner1 = 108 * scale;
    const outer1 = 182 * scale;
    const inner2 = 196 * scale;
    const outer2 = 246 * scale;
    const centerRadius = 102 * scale;
  ...
  function renderSyopTrend() {
    const container = document.getElementById('syop-trend-chart');
    if (!container) return;
    container.innerHTML = '';

    const legend = createEl('div', { class: 'syop-line-legend syop-trend-legend' });
    ...
    const margin = width < 520
      ? { top: 74, right: 22, bottom: 54, left: 60 }
      : { top: 78, right: 32, bottom: 62, left: 76 };
    ...
    const stepCount = Math.max(1, SYOP_DATA.length - 1);
    const xStep = chartWidth / stepCount;
    const yMax = Math.max(...SYOP_DATA.map((row) => Math.max(...SYOP_SERIES.map((series) => row[series.key] || 0))));
    const niceMax = Math.max(10, Math.ceil(yMax / 5) * 5);
    const tickGap = niceMax > 40 ? 10 : 5;
    const ticks = [];
    for (let value = 0; value <= niceMax + 0.0001; value += tickGap) {
      ticks.push(value);
    }
    ...
    ticks.forEach((tick) => {
      ...
      g.appendChild(createSVG('line', {
        ...
        y1: y,
        y2: y,
        stroke: tick === 0 ? 'rgba(255,255,255,0.18)' : colors.grid
      }));
    ...
    SYOP_DATA.forEach((row, index) => {
      const x = index * xStep;
      const labelX = index === 0 ? x + 6 : index === SYOP_DATA.length - 1 ? x - 6 : x;
      g.appendChild(createSVG('line', {
        x1: x,
        x2: x,
        y1: 0,
      ...
      g.appendChild(createSVG('text', {
        ...
        fill: colors.subtext,
        'font-size': '12',
        'text-anchor': index === 0 ? 'start' : index === SYOP_DATA.length - 1 ? 'end' : 'middle'
    ...
    SYOP_SERIES.forEach((series) => {
      const points = SYOP_DATA.map((row, index) => {
        ...
        return {
          x: index * xStep,
          y: chartHeight - (value / niceMax) * chartHeight,
          value,
          label: row.SYOP
        };
      });
      ...
      points.forEach((point) => {
        const halo = createSVG('circle', {
          ...
          cy: point.y,
          r: width < 520 ? 7.2 : 8,
          fill: hexToRgba(series.color, 0.24),
          stroke: 'none'
        });
        g.appendChild(halo);
        ...
        title.textContent = `${series.label} — ${point.label}: ${point.value.toFixed(1)}%`;
    ...
    g.appendChild(createSVG('text', {
      ...
      'font-size': '12',
      'font-weight': '600'
    }, document.createTextNode('% of positional qualifiers')));
