(function () {
  ...
  const DRAFT_SERIES = [
    { key: 'QB', color: DRAFT_COLORS.qb },
    { key: 'RB', color: DRAFT_COLORS.rb },
    { key: 'TE', color: DRAFT_COLORS.te },
    { key: 'WR', color: DRAFT_COLORS.wr }
  ];
  ...
  function renderSunburst() {
    ...
    const textStroke = 'rgba(6, 9, 18, 0.78)';
    const fontSize = (value, floor = 10) => Math.max(value * scale, floor);
    ...
    ring1Segments.forEach((segment) => {
      ...
      const text = createSVG('text', {
        ...
        'paint-order': 'stroke',
        stroke: textStroke,
        'stroke-width': (0.55 * scale).toFixed(3)
      });
    ...
    ring2Segments.forEach((segment) => {
      ...
      const stat = segment.node.subtitle || '';
      if (stat) {
  ...
  function renderBarChart() {
    ...
    const margin = width < 520
      ? { top: 32, right: 20, bottom: 52, left: 56 }
      : { top: 38, right: 28, bottom: 60, left: 68 };
  ...
  function renderDraftOverall() {
    ...
    const margin = width < 540
      ? { top: 26, right: 14, bottom: 48, left: 54 }
      : { top: 32, right: 22, bottom: 56, left: 64 };
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const svg = createSVG('svg', {
      viewBox: `0 0 ${width} ${height}`,
    ...
    gradient.appendChild(createSVG('stop', { offset: '0%', 'stop-color': DRAFT_COLORS.barTop }));
    gradient.appendChild(createSVG('stop', { offset: '60%', 'stop-color': DRAFT_COLORS.barMid }));
    gradient.appendChild(createSVG('stop', { offset: '100%', 'stop-color': DRAFT_COLORS.barBottom }));
    defs.appendChild(gradient);
    svg.appendChild(defs);
    ...
    DRAFT_OVERALL.forEach((row, index) => {
      const baseX = index * groupWidth;
      const barWidth = Math.max(18, Math.min(width < 540 ? groupWidth * 0.82 : groupWidth * 0.68, 64));
  ...
  function renderDraftPositional() {
    ...
    const margin = width < 560
      ? { top: 54, right: 20, bottom: 48, left: 62 }
      : { top: 50, right: 26, bottom: 56, left: 68 };
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const svg = createSVG('svg', {
      viewBox: `0 0 ${width} ${height}`,
