(function () {
  ...
  const SUNBURST_NODES = [
    { id: 'root', parent: null, label: 'AVG [Mean | Λ]', subtitle: 'Majority [Mode | M]', value: 51.6 },
    { id: 'qb', parent: 'root', label: 'QB', subtitle: null, value: 16.46, series: 'QB' },
    { id: 'qb-prime-lambda', parent: 'qb', label: 'SPΛ', display: '7.2', value: 6.5 },
    { id: 'qb-breakout-lambda', parent: 'qb', label: 'BOΛ', display: '2.3', value: 2.49 },
    { id: 'qb-prime-mode', parent: 'qb', label: 'SPM', display: '6.0', value: 5.35 },
    { id: 'qb-baseline-mode', parent: 'qb', label: 'BOM', display: '1.0', value: 2.1 },
    { id: 'rb', parent: 'root', label: 'RB', subtitle: null, value: 9.8, series: 'RB' },
    { id: 'rb-prime-lambda', parent: 'rb', label: 'SPΛ', display: '3.4', value: 3.31 },
    { id: 'rb-breakout-lambda', parent: 'rb', label: 'BOΛ', display: '2.2', value: 2.5 },
    { id: 'rb-prime-mode', parent: 'rb', label: 'SPM', display: '0.7', value: 1.89 },
    { id: 'rb-baseline-mode', parent: 'rb', label: 'BOM', display: '1.7', value: 2.1 },
    { id: 'wr', parent: 'root', label: 'WR', subtitle: null, value: 12.82, series: 'WR' },
    { id: 'wr-prime-lambda', parent: 'wr', label: 'SPΛ', display: '4.9', value: 4.92 },
    { id: 'wr-breakout-lambda', parent: 'wr', label: 'BOΛ', display: '2.9', value: 2.84 },
    { id: 'wr-prime-mode', parent: 'wr', label: 'SPM', display: '3.0', value: 3 },
    { id: 'wr-baseline-mode', parent: 'wr', label: 'BOM', display: '2.0', value: 2 },
    { id: 'te', parent: 'root', label: 'TE', subtitle: null, value: 12.5, series: 'TE' },
    { id: 'te-prime-lambda', parent: 'te', label: 'SPΛ', display: '4.0', value: 4.01 },
    { id: 'te-breakout-lambda', parent: 'te', label: 'BOΛ', display: '3.5', value: 3.49 },
    { id: 'te-prime-mode', parent: 'te', label: 'SPM', display: '2.0', value: 2 },
    { id: 'te-baseline-mode', parent: 'te', label: 'BOM', display: '3.0', value: 3 }
  ];
  ...
  function measureWidth(container, { min = 320, max = 900 } = {}) {
    const viewport = typeof window !== 'undefined' ? window.innerWidth : max;
    const parentWidth = container.parentElement ? container.parentElement.clientWidth : 0;
    const boundsWidth = container.clientWidth || 0;
    const basis = Math.max(boundsWidth, parentWidth, viewport - 80);
    return Math.min(Math.max(basis, min), max);
  }

  function smoothLinePath(points, tension = 0.5) {
  ...
  function renderSunburst() {
    ...
    ring2Segments.forEach((segment) => {
      ...
      const displayValue = segment.node.display || segment.node.subtitle;
      if (displayValue) {
        label.appendChild(createSVG('tspan', {
          ...
          'font-weight': '600',
          fill: colors.subtext
        }, document.createTextNode(`(${displayValue} yrs)`)));
      }
    ...
    const titleTop = createSVG('text', {
      ...
      stroke: 'rgba(11, 14, 22, 0.65)',
      'stroke-width': '0.55'
    }, document.createTextNode(root?.label || 'Mean Λ'));
    ...
    const titleBottom = createSVG('text', {
      ...
      stroke: 'rgba(11, 14, 22, 0.65)',
      'stroke-width': '0.55'
    }, document.createTextNode(root?.subtitle || 'Mode M'));
    ...
    svg.appendChild(createSVG('text', {
      ...
      'font-weight': '600',
      'text-anchor': 'middle'
    }, document.createTextNode('SYOP cohort • QB · RB · WR · TE')));
  ...
  function renderBarChart() {
    ...
    const width = measureWidth(container, { min: 340, max: 900 });
    ...
    SYOP_DATA.forEach((row, index) => {
      ...
      if (barState.stacked) {
        const barWidth = Math.min(isCompact ? 40 : 56, groupWidth * 0.6);
        let cumulative = 0;
        activeSeries.forEach((series) => {
          ...
          const rect = createSVG('rect', {
            ...
            fill: hexToRgba(series.color, 0.92),
            rx: 8,
            ry: 8
          });
          g.appendChild(rect);
        ...
        const barWidth = Math.min(isCompact ? 22 : 32, (groupWidth - innerGap) / activeSeries.length);
  ...
  function renderDraftOverall() {
    ...
    const width = measureWidth(container, { min: 340, max: 860 });
    const isCompact = width < 620;
    const height = isCompact ? 260 : 320;
    const margin = {
      top: isCompact ? 30 : 36,
      right: isCompact ? 18 : 24,
      bottom: isCompact ? 46 : 56,
      left: isCompact ? 50 : 64
    };
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const svg = createSVG('svg', {
    ...
    const ticks = [];
    const tickStep = niceMax > 60 ? 20 : 10;
    for (let value = 0; value <= niceMax + 0.0001; value += tickStep) {
      ticks.push(value);
    }
    ...
    DRAFT_OVERALL.forEach((row, index) => {
      ...
      const barWidth = Math.min(isCompact ? 42 : 58, groupWidth * 0.62);
  ...
  function renderDraftPositional() {
    ...
    const width = measureWidth(container, { min: 360, max: 860 });
    const isCompact = width < 640;
    const height = isCompact ? 320 : 360;
    const margin = {
      top: isCompact ? 42 : 48,
      right: isCompact ? 26 : 32,
      bottom: isCompact ? 52 : 60,
      left: isCompact ? 60 : 72
    };
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const svg = createSVG('svg', {
    ...
    DRAFT_SERIES.forEach((series) => {
      ...
      const pathData = smoothLinePath(points, isCompact ? 0.6 : 0.75);
      const path = createSVG('path', {
        d: pathData,
        fill: 'none',
        stroke: series.color,
