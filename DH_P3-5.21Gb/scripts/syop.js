(function () {
  ...
  function renderSunburst() {
    ...
    const svg = createSVG('svg', {
      ...
      role: 'img',
      'aria-label': 'SYOP mean and mode distribution sunburst',
    ...
    ring2Segments.forEach((segment) => {
      ...
      const totalLines = words.length + (paren ? 1 : 0);
      const text = createSVG('text', {
        x: center.x,
        y: center.y - (totalLines - 1) * 8,
        fill: colors.text,
        'text-anchor': 'middle',
  ...
  function renderGauges() {
    ...
    GAUGES.forEach((gauge) => {
      ...
      const gaugeWrapper = createEl('div', { class: 'syop-gauge-card' }, svg, label);
  ...
  function renderGaugeSVG(gauge) {
    ...
    const svg = createSVG('svg', {
      ...
      role: 'img',
      'aria-label': `${gauge.key} average SYOP ${gauge.value.toFixed(2)}`
    });
    ...
    ticks.forEach((tick) => {
      ...
      const outer = polar(cx, cy, radius + trackWidth / 2 + 2, angle);
      const line = createSVG('line', {
        x1: inner.x,
        y1: inner.y,
        x2: outer.x,
    ...
    const unitsText = createSVG('text', {
      x: cx,
      y: cy - radius * 0.32,
      fill: colors.subtext,
      'font-size': '12',
  ...
  function renderDraftPositional() {
    ...
    const width = 760;
    const height = 320;
    const margin = { top: 40, right: 28, bottom: 52, left: 66 };
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const svg = createSVG('svg', {
      viewBox: `0 0 ${width} ${height}`,
