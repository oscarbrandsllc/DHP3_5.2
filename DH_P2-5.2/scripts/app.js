...
function showLegend(){ try{ document.getElementById('legend-section')?.classList.remove('hidden'); }catch(e){} }
    ...
    hideLegend();
        ...
        function calculatePlayerStatsAndRanks(playerId) {
            ...
            const HEADER_TO_INTERNAL_KEY_MAP = {
                'paATT': 'pass_att', 'CMP': 'pass_cmp', 'paYDS': 'pass_yd', 'paTD': 'pass_td', 'pa1D': 'pass_fd',
                'IMP/G': 'imp_per_g', 'paRTG': 'pass_rtg', 'pIMP': 'pass_imp', 'pIMP/A': 'pass_imp_per_att',
                'INT': 'pass_int', 'SAC': 'pass_sack', 'TTT': 'ttt', 'PRS%': 'prs_pct', 'CAR': 'rush_att',
                'ruYDS': 'rush_yd', 'YPC': 'ypc', 'ruTD': 'rush_td', 'ru1D': 'rush_fd', 'MTF': 'mtf',
                'ELU': 'elu', 'YCO': 'rush_yac', 'YCO/A': 'yco_per_att', 'MTF/A': 'mtf_per_att',
                'TGT': 'rec_tgt', 'REC': 'rec', 'recYDS': 'rec_yd', 'recTD': 'rec_td', 'rec1D': 'rec_fd',
                'YAC': 'rec_yar', 'YPR': 'ypr', 'RR': 'rr', 'TS%RR': 'ts_per_rr', 'YPRR': 'yprr',
                '1DRR': 'first_down_rec_rate', 'IMP': 'imp', 'FUM': 'fum', 'SNP%': 'snp_pct'
            };
            ...
            for (const week in state.weeklyStats) {
                ...
                for (const pId in weeklyData) {
                    if (allPlayers[pId]) { // Make sure the player exists in our list
                        const weeklyPlayerStats = weeklyData[pId];
                        const translatedStats = {};
                        for (const headerKey in weeklyPlayerStats) {
                            const internalKey = HEADER_TO_INTERNAL_KEY_MAP[headerKey] || headerKey;
                            translatedStats[internalKey] = weeklyPlayerStats[headerKey];
                        }

                        const points = calculateFantasyPoints(translatedStats, scoringSettings);
        ...
        function renderGameLogs(gameLogs, player, playerRanks) {
            ...
            const qbStatOrder = [
                'FPTS',
                'paATT', 'CMP', 'paYDS', 'paTD', 'pa1D', 'INT', 'SAC',
                'paRTG', 'pIMP', 'pIMP/A', 'TTT', 'PRS%',
                'CAR', 'ruYDS', 'ruTD',
                'FUM'
            ];
            ...
            const rbStatOrder = [
                'FPTS',
                'SNP%',
                'CAR', 'ruYDS', 'ruTD', 'ru1D', 'YPC',
                'ELU', 'MTF', 'MTF/A', 'YCO', 'YCO/A',
                'TGT', 'REC', 'recYDS', 'recTD', 'rec1D', 'YAC',
                'FUM'
            ];
            ...
            const wrTeStatOrder = [
                'FPTS',
                'SNP%',
                'TGT', 'REC', 'recYDS', 'recTD', 'rec1D',
                'YAC', 'YPR', 'RR', 'YPRR', 'TS%RR', '1DRR',
                'CAR', 'ruYDS', 'ruTD', 'YPC',
                'FUM'
            ];
            ...
            else orderedStatKeys = [
                'FPTS', 'paATT', 'CMP', 'paYDS', 'paTD', 'pa1D', 'IMP/G', 'paRTG', 'pIMP', 'pIMP/A',
                'CAR', 'ruYDS', 'YPC', 'ruTD', 'ru1D', 'TTT', 'PRS%', 'MTF', 'MTF/A', 'YCO', 'YCO/A',
                'TGT', 'REC', 'recYDS', 'recTD', 'rec1D', 'YAC', 'YPR', 'YPRR', 'TS%RR', 'RR', 'FUM', 'SNP%'
            ];
            ...
            gameLogs.sort((a, b) => parseInt(a.week) - parseInt(b.week)).forEach(weekStats => {
                ...
                for (const key of orderedStatKeys) {
                    ...
                    } else if (['YCO/A', 'MTF/A', 'YPC', 'TTT', 'YPR', 'YPRR', '1DRR'].includes(key)) {
                        displayValue = value.toFixed(2);
                    } else if (['pIMP/A', 'PRS%', 'SNP%', 'TS%RR'].includes(key)) {
            ...
            if (gameLogsWithData.length > 0) {
                ...
                const gamesPlayed = seasonTotals?.games_played ?? '0';
                ...
                for (const key of orderedStatKeys) {
                    ...
                    if (typeof value !== 'number') {
                        ...
                        displayValue = Number.isInteger(value) ? String(value) : Number(value || 0).toFixed(2).replace(/\.00$/, '');
                    }
        ...
        function renderPlayerComparison(players) {
            ...
            for (const statKey of orderedStatKeys) {
                ...
                for (let i = 0; i < players.length; i++) {
                    ...
                    if (statKey === 'FPTS') {
                        calculatedValue = seasonTotals?.fpts_ppr || 0;
                    } else {
                        calculatedValue = seasonTotals?.[statKey] || 0;
                    }

                    if (typeof calculatedValue !== 'number') {
                        displayValue = calculatedValue || '0';
                    } else if (['YCO/A', 'MTF/A', 'YPC', 'TTT', 'YPR', 'YPRR', '1DRR'].includes(statKey)) {
                        displayValue = calculatedValue.toFixed(2);
                    } else if (['pIMP/A', 'PRS%', 'SNP%', 'TS%RR'].includes(statKey)) {
                        displayValue = formatPercentage(calculatedValue);
                    } else {
                        displayValue = Number.isInteger(calculatedValue) ? String(calculatedValue) : Number(calculatedValue || 0).toFixed(2).replace(/\.00$/, '');
                    }
